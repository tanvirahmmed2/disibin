...
// GET coupons (Admin/Manager sees all, Public can validate one)
export async function GET(req) {
    try {
        const { searchParams } = new URL(req.url);
        const code = searchParams.get('code');

        // If code is provided, validate it (Public)
        if (code) {
        ...
        // Otherwise, list all (Manager only)
...
export async function POST(req) {
    try {
        ...
        if (!code || !discount_type || discount_value === undefined) {
            return NextResponse.json({ success: false, message: "Missing required fields (code, discount_type, discount_value)" }, { status: 400 });
        }
...
export async function DELETE(req) {
    try {
        ...
        const deletedCoupon = await deleteCoupon(couponId, auth.data.id);
        if (!deletedCoupon) return NextResponse.json({ success: false, message: "Coupon not found" }, { status: 404 });
