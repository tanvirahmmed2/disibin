...
export default function TeamRefundPolicyPage() {
  ...
  const fetchRefunds = async () => {
    setLoading(true);
    try {
      const res = await axios.get('/api/team/refund-policy');
      if (res.data.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        const res = await axios.put('/api/team/refund-policy', {
          id: editingItem.id,
          ...formData
        });
        if (res.data.success) {
        ...
        const res = await axios.post('/api/team/refund-policy', formData);
        if (res.data.success) {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this Refund Policy record?')) return;
    setDeletingId(id);
    try {
      const res = await axios.delete(`/api/team/refund-policy?id=${id}`);
      if (res.data.success) {
    ...
    <div className="p-4 sm:p-8 max-w-7xl mx-auto min-h-screen pb-24">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 bg-gradient-to-r from-slate-900 via-primary to-slate-800 p-6 sm:p-8 rounded-3xl text-white shadow-xl">
        <div>
          ...
          <p className="text-slate-300 text-sm mt-1 max-w-xl">
            Create, update, and publish official Refund Policy & SLA conditions. Restricted exclusively to authorized managers.
          </p>
        </div>
        ...
        >
          <FiPlus className="w-5 h-5 text-primary" /> Create Refund Policy
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 px-4 bg-white rounded-3xl border border-slate-100 shadow-sm">
          ...
          <h3 className="text-lg font-bold text-slate-800">No Refund Policy records</h3>
          <p className="text-slate-500 text-sm max-w-md mx-auto mt-1 mb-6">
            There are no Refund Policy entries created yet. Click below to add one.
          </p>
          <button
            onClick={handleOpenCreate}
          ...
          >
            <FiPlus className="w-4 h-4" /> Add Refund Policy
          </button>
        </div>
      ) : (
          ...
          <div className="bg-white w-full max-w-3xl rounded-3xl shadow-2xl border border-slate-100 overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-slate-50/50">
              <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                ...
                {editingItem ? 'Edit Refund Policy' : 'Create New Refund Policy'}
              </h3>
              <button
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. Disibin Refund Policy"
              ...
              <div className="flex items-center gap-3 pt-2">
                <input
                  type="checkbox"
                  id="is_published_refund"
                  checked={formData.is_published}
                  onChange={(e) => setFormData({ ...formData, is_published: e.target.checked })}
                  className="w-4 h-4 text-sky-600 rounded focus:ring-sky-500"
                />
                <label htmlFor="is_published_refund" className="text-sm font-medium text-slate-700 cursor-pointer">
