...
export default function TeamPrivacyPolicyPage() {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this Privacy Policy record?')) return;
    setDeletingId(id);
    try {
    ...
    <div className="p-4 sm:p-8 max-w-7xl mx-auto min-h-screen pb-24">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 bg-gradient-to-r from-slate-900 via-primary to-slate-800 p-6 sm:p-8 rounded-3xl text-white shadow-xl">
        <div>
          ...
          <p className="text-slate-300 text-sm mt-1 max-w-xl">
            Create, update, and publish official Privacy Policy documents. Restricted exclusively to authorized managers.
          </p>
        </div>
        ...
        >
          <FiPlus className="w-5 h-5 text-primary" /> Create Privacy Policy
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 px-4 bg-white rounded-3xl border border-slate-100 shadow-sm">
          ...
          <h3 className="text-lg font-bold text-slate-800">No Privacy Policy records</h3>
          <p className="text-slate-500 text-sm max-w-md mx-auto mt-1 mb-6">
            There are no Privacy Policy entries created yet. Click below to add one.
          </p>
          <button
            onClick={handleOpenCreate}
          ...
          >
            <FiPlus className="w-4 h-4" /> Add Privacy Policy
          </button>
        </div>
      ) : (
          ...
          <div className="bg-white w-full max-w-3xl rounded-3xl shadow-2xl border border-slate-100 overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-slate-50/50">
              <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                ...
                {editingItem ? 'Edit Privacy Policy' : 'Create New Privacy Policy'}
              </h3>
              <button
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. Disibin Privacy Policy"
              ...
              <div className="flex items-center gap-3 pt-2">
                <input
                  type="checkbox"
                  id="is_published_privacy"
                  checked={formData.is_published}
                  onChange={(e) => setFormData({ ...formData, is_published: e.target.checked })}
                  className="w-4 h-4 text-sky-600 rounded focus:ring-sky-500"
                />
                <label htmlFor="is_published_privacy" className="text-sm font-medium text-slate-700 cursor-pointer">
