...
const ClientTickets = () => {
    ...
    const columns = [
        ...
        { label: 'Status', key: 'status', render: (row) => (
            <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider
                ${row.status === 'open' ? 'bg-primary/10 text-primary-dark' : 
    ...
    const actions = (row) => (
        <div className="flex gap-2">
            <button className="flex items-center gap-1 p-2 hover:bg-primary/5 rounded-lg text-primary transition-all font-bold text-xs">
