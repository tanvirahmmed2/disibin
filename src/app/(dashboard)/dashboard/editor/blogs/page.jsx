    ...
    return (
        <div className="space-y-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <Link href="/dashboard/editor/blogs/new" className="flex items-center gap-2 bg-slate-900 text-white px-8 py-4 rounded-2xl font-black uppercase tracking-widest text-[11px] shadow-xl shadow-slate-900/10 hover:bg-emerald-500 transition-all active:scale-95">
                    <RiAddLine size={24} />
...

               {
                blogs.length===0?<div className='w-full h-full flex items-center justify-center p-20 font-semibold'>
                    ...
                    {
                        blogs.map((blog)=>(
                            <div key={blog._id} className='w-full flex flex-row items-center justify-between p-4 shadow even:bg-slate-100'>
                                <p>{blog.title}</p>
                                <div className='w-auto flex flex-row items-center justify-center gap-4 text-xl'>
                                    <Link href={`/dashboard/editor/blogs/${blog.slug}`}><MdEditDocument/></Link>
                                    <button onClick={()=>handleDelete(blog._id)}><MdDeleteOutline/></button>
                                </div>
                            </div>
                        ))
                    }
