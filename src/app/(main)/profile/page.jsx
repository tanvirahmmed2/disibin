...
import { RiUserLine,  RiLogoutBoxRLine, RiShieldUserLine, RiMapPin2Line, RiMailLine, RiPhoneLine } from 'react-icons/ri';
  ...
  if (!userData) return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50">
      <div className="bg-white p-12 rounded-[3rem] shadow-2xl shadow-slate-200 border border-slate-50 text-center max-w-md">
        ...
        <Link href="/login" className="px-12 py-5 bg-slate-900 text-white font-black uppercase tracking-widest text-[11px] rounded-xl hover:bg-emerald-600 transition-all inline-block shadow-xl shadow-slate-900/10">Secure Login</Link>
      </div>
    </div>
  );
  ...
  return (
    <div className="w-full min-h-screen bg-white py-24 px-6 overflow-hidden relative">
      <div className="max-w-6xl mx-auto space-y-12 relative z-10">
        ...
        <div className="bg-emerald-900 rounded-[3.5rem] p-12 md:p-16 flex flex-col md:flex-row items-center gap-12 relative overflow-hidden text-white">
                ...
                <div className="space-y-4">
                    <div className="flex flex-col md:flex-row md:items-center gap-6">
                        ...
                        <span className={`px-5 py-2 rounded-2xl text-[10px] font-black uppercase tracking-widest backdrop-blur-md border border-white/10
                            ${userData.role === 'admin' ? 'bg-emerald-600/20 text-emerald-600' : 'bg-emerald-600/20 text-emerald-600'}`}>
                    ...
                    <p className="text-white/40 font-bold uppercase text-[11px] tracking-[0.3em] flex items-center justify-center md:justify-start gap-2">
                        <RiShieldUserLine size={16} className="text-emerald-600" /> Verified Profile 
                        <span className="w-1.5 h-1.5 rounded-full bg-emerald-600/50 animate-pulse ml-2"></span>
                    </p>
                </div>

                <div className="flex flex-wrap justify-center md:justify-start gap-8">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-2xl bg-white/5 flex items-center justify-center text-emerald-600"><RiMailLine size={18} /></div>
                    ...
                    {userData.phone && (
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-2xl bg-white/5 flex items-center justify-center text-emerald-600"><RiPhoneLine size={18} /></div>
                            <div className="flex flex-col">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            <div className="bg-white p-12 rounded-[3.5rem] border border-slate-100 shadow-sm space-y-10">
                ...
                <div className="grid grid-cols-2 gap-8">
                    <div className="space-y-1">
                        ...
                        <p className="text-slate-800 font-bold flex items-center gap-2"><FaGlobe className="text-emerald-600" /> {userData.country || 'Not Specified'}</p>
                    </div>
                    <div className="space-y-1">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">State / Region</p>
                        <p className="text-slate-800 font-bold flex items-center gap-2"><FaMapMarkerAlt className="text-emerald-600" /> {userData.state || 'Not Specified'}</p>
                    </div>
                    <div className="space-y-1 text-nowrap overflow-hidden">
                        ...
                        <p className="text-slate-800 font-bold flex items-center gap-2"><FaCity className="text-emerald-600" /> {userData.city || 'Not Specified'}</p>
                    </div>
                    <div className="space-y-1">
            ...
            <div className="flex flex-col gap-10">
                    ...
                    <div className="space-y-8">
                        <div className="flex items-center justify-between p-6 bg-white rounded-3xl shadow-sm border border-slate-100">
                            <div className="flex items-center gap-4">
                                <div className="w-10 h-10 bg-emerald-600/10 text-emerald-600 rounded-2xl flex items-center justify-center"><FaCalendarAlt size={16} /></div>
                                <div className="flex flex-col">
                        ...
                        <div className="flex items-center justify-between p-6 bg-white rounded-3xl shadow-sm border border-slate-100">
                            <div className="flex items-center gap-4">
                                <div className="w-10 h-10 bg-emerald-600 text-emerald-600 rounded-2xl flex items-center justify-center"><RiShieldUserLine size={20} /></div>
                                <div className="flex flex-col">
                                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Account Status</span>
                                    <span className={`text-sm font-black uppercase tracking-wider ${userData.isActive ? 'text-emerald-600/50' : 'text-emerald-600'}`}>
                ...
                <div className="flex gap-4">
                    <Link 
                        href="/dashboard"
                        className="flex-1 px-12 py-6 bg-slate-900 text-white font-black uppercase tracking-widest text-[11px] rounded-[2.5rem] hover:bg-emerald-600 transition-all flex items-center justify-center gap-3 active:scale-95 shadow-xl shadow-slate-900/10"
                    >
                        Management Console <span className="text-xl">→</span>
                    </Link>
                    <button 
                        onClick={handleLogout}
                        className="w-24 h-24 bg-white border border-slate-100 text-emerald-600 hover:text-white rounded-[2.5rem] flex flex-col items-center justify-center gap-1 hover:bg-emerald-600 transition-all active:scale-95 shadow-sm"
                    >
