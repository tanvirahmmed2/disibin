        ...
        <form onSubmit={handleSubmit} className='w-full flex flex-col items-center gap-4 p-1 sm:p-4'>
            ...
            <div className='w-full flex flex-col md:flex-row items-center justify-between gap-3'>
                <div className='flex flex-col gap-1 w-full'>
                    ...
                    <input name="price" type="number" value={formData.price} onChange={handleChange} className='w-full px-3 p-1 border border-primary/30 outline-none' required />
                </div>
                <div className='flex flex-col gap-1 w-full'>
                    <label className='font-bold text-gray-700'>Discount</label>
                    <input name="discount" type="number" value={formData.discount} onChange={handleChange} className='w-full px-3 p-1 border border-primary/30 outline-none'/>
                </div>
            </div>

            <div className='flex flex-col gap-1 w-full'>
                <label className='font-bold text-gray-700'>Category</label>
                <input name="category" value={formData.category} onChange={handleChange} className='w-full px-3 p-1 border border-primary/30 outline-none' required />
            </div>

            <div className='flex flex-col gap-1 w-full'>
                <label className='font-bold text-gray-700'>Features (comma separated)</label>
                <input name="features" value={formData.features} onChange={handleChange} className='w-full px-3 p-1 border border-primary/30 outline-none' />
            </div>

            <div className='flex items-center gap-2 py-2'>
                <input type="checkbox" name="is_popular" checked={formData.is_popular} onChange={handleChange} id="popular-package" className='w-5 h-5 accent-primary' />
            ...
            <button type="submit" className='bg-primary text-white p-3 rounded-md hover:bg-primary-dark font-bold transition-colors'>
                Update Package
            </button>
        </form>
    )
