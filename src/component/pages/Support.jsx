    ...
    <section className='w-full py-24 bg-white'>
      <div className='container-custom grid grid-cols-1 lg:grid-cols-2 gap-16 items-start'>
        ...
        <div className='space-y-12'>
            ...
            <div className='space-y-6 pt-4'>
                <div className='flex items-center gap-4 group'>
                    <div className='w-12 h-12 bg-slate-50 rounded-xl flex items-center justify-center text-slate-400 group-hover:bg-emerald-500 group-hover:text-white transition-all'>
        ...
        <motion.div 
            ...
            viewport={{ once: true }}
            className='p-10 bg-slate-50/50 border border-slate-100 rounded-2xl space-y-8'
        >
            <div className='flex items-center gap-3'>
                <RiMessage2Line size={24} className='text-emerald-500' />
                <h3 className='text-xl font-bold text-slate-900 tracking-tight'>Direct Inquiry</h3>
            </div>

            <form onSubmit={handleSubmit} className='space-y-5'>
                <div className='grid grid-cols-1 md:grid-cols-2 gap-5'>
                    <div className='space-y-1.5'>
                        <label className='text-[10px] font-semibold uppercase tracking-widest text-slate-400 ml-4'>Full Name</label>
                        <input name='name' required onChange={handleChange} value={formData.name} className='input-standard' />
                    </div>
                    <div className='space-y-1.5'>
                        <label className='text-[10px] font-semibold uppercase tracking-widest text-slate-400 ml-4'>Email Address</label>
                        <input type="email" name='email' required onChange={handleChange} value={formData.email} className='input-standard' />
                    </div>
                </div>
                <div className='space-y-1.5'>
                    <label className='text-[10px] font-semibold uppercase tracking-widest text-slate-400 ml-4'>Subject</label>
                    <input name='subject' required onChange={handleChange} value={formData.subject} className='input-standard' />
                </div>
                <div className='space-y-1.5'>
                    <label className='text-[10px] font-semibold uppercase tracking-widest text-slate-400 ml-4'>Message</label>
                    <textarea name="message" required onChange={handleChange} value={formData.message} className='input-standard min-h-32 resize-none' />
                </div>
                
                <button 
                    disabled={loading}
                    type='submit' 
                    className='w-full bg-slate-900 hover:bg-emerald-600 rounded-xl text-white py-5 font-semibold flex items-center justify-center gap-3 transition-all active:scale-[0.98]'
                >
